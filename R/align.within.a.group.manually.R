align.within.a.group.manually<-function(dat.group, refp.idx, manual.lag.vec, padtyp, printQ=FALSE)
{                              
  
if(printQ==TRUE) {
     print("***************************************")
     print(paste("Manually Aligning All Profiles to CHOSEN reference profile#",refp.idx,sep=""))
     print("***************************************")
    }   
...
for(i in 1:nrow(dat.group))
 {
  if(i !=refp.idx)
   {
    movp<-dat.group[i,]
    info.list<-manual.align.mov.to.ref.pad.both(refp,movp,manual.lag.vec[i],padtyp,printQ)
    pdref<-info.list[[1]]
    pdmov<-info.list[[2]]
