#################################################################
#Align arbitrary mov to reference, padding both
#################################################################
align.mov.to.ref.pad.both<-function(ref,mov,lagmax,padtyp,printQ=FALSE)
{
...
######################################################################################
#Align arbitrary mov to reference, padding both. Use FFT base correlation algorithm 
######################################################################################
align.mov.to.ref.pad.both2<-function(ref,mov,lagmax,padtyp,printQ=FALSE)
{
  ...
  #-----------FIXED HERE-------------
  #croscor<-ccf(refp,movp,lag.max=lagmax,plot=FALSE,na.action=na.omit) #Using Cross-Correlation for alignment
  croscor<-ccf.rfftw(refp,movp,max.lag=lagmax)
  
  lags<-croscor[[2]]
  max.corr.idx<-which(croscor[[1]]==max(croscor[[1]]))
  maxlag<-lags[max.corr.idx]
  dif<-length(refp)-length(movp)
  #-------------------------------
  ...
  if(printQ==TRUE)
  {
    ...
    print(paste("  Index of max corr: ",max.corr.idx,sep=""))
    print(paste("    Max corr at lag: ",maxlag,sep=""))
    print(paste("           Max Corr: ",max(croscor[[1]]),sep=""))
  }
