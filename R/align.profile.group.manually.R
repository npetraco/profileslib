########################################
#Manually align profiles within a group
########################################
align.profile.group.manually<-function(profile.group,lagmax,lag.vec,printQ=FALSE)
{
  ...
  if(i!=refp.idx)
   {
    ...
    max.corr.idx<-which(lags==lag.vec[i])  
    ...
    if(printQ==TRUE)
     {
      ...
      print(paste("Index of CHOOSEN corr: ",max.corr.idx,sep=""))
      print(paste("CHOOSEN lag: ",maxlag,sep=""))
      print(paste("Corr at CHOOSEN lag: ",croscor$acf[,,1][max.corr.idx],sep=""))
      #print("================================")
     }
...
for(i in 1:dim(sftmat)[1])
 {
  if(sftmat[i,2]<0 || (sftmat[i,2]>0 && sftmat[i,3]>0))
   {
   	#print("*CASE1")
   	modmovp<-c(rep(NA,sftmat[i,6]),movplist[[i]],rep(NA,sftmat[i,7])) #prepend and append appropriate amounts to mov profile
...
refp<-c(rep(NA,mp),refp,rep(NA,ma)) #finally pad reference 
